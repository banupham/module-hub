...
class ModuleManager:
    def __init__(self, root: Path, registry_path: Path, config_path: Path):
        ...
        self.registry = json.loads(registry_path.read_text(encoding="utf-8"))["modules"]
    ...
    def start(self, module_id: str, mode: str = "default", options: dict[str, Any] | None = None) -> dict[str, Any]:
        options = options or {}
        with self.lock:
            ...
            env = os.environ.copy()
            env.update({str(k): str(v) for k, v in self.registry[module_id].get("env", {}).items()})
            env.update({str(k): str(v) for k, v in options.get("env", {}).items()})

            log_dir = self.root / "logs"
            log_dir.mkdir(exist_ok=True)
            log_handle = open(log_dir / f"{module_id}.log", "a", encoding="utf-8", errors="replace")
            ...
            return {"ok": True, "pid": process.pid, "command": command, "cwd": str(cwd)}
